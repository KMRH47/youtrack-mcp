...
class YouTrackMCPServer:
    ...
    def _generate_tool_schema(
        ...
        for param_name, param in sig.parameters.items():
            ...
            # Handle common types
            if param_type in (int, float) or (
                hasattr(param_type, "__origin__")
                and param_type.__origin__ in (int, float)
            ):
                schema_type = "number"
                ...
                hasattr(param_type, "__origin__")
                and param_type.__origin__ in (dict, Dict)
            ):
                schema_type = "object"
            elif param_type in (list, List) or (
                hasattr(param_type, "__origin__")
                and param_type.__origin__ in (list, List)
            ):
                schema_type = "array"

            # Get description from parameter_descriptions or default
            param_description = parameter_descriptions.get(
    ...
    def register_tool(
        ...
        # Create SIMPLIFIED description for LLMs (no verbose parameter docs)
    ...
    def _wrap_tool_function(
        self, func: Callable, name: str, should_stream: bool = True
    ) -> Callable:
        """
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                    ...
                    if isinstance(kwargs_value, str):
                        if kwargs_value.startswith(
                            ...
                            try:
                                ...
                                logger.warning(
                                    f"Failed to parse kwargs as JSON: {kwargs_value}"
                                )
                        else:
                ...
                logger.debug(
                    f"Tool {name} processed parameters: args={processed_args}, kwargs={processed_kwargs}"
                )
                ...
                if name in ("get_issue", "add_comment", "create_issue"):
                    ...
                    if (
                        name == "get_issue"
                        and not processed_kwargs.get("issue_id")
                        and processed_args
                    ):
                ...
                if isinstance(result, dict):
                    result_with_meta = {"tool": name, "result": result}
    ...
    def _extract_real_kwargs(self, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if (
            ...
            try:
                ...
                if kwargs["args"].strip().startswith("{") and kwargs[
                    "args"
                ].strip().endswith("}"):
                ...
                logger.warning(
                    f"Failed to parse args as JSON: {kwargs['args']}. Error: {str(e)}"
                )
                if kwargs["args"].strip():
                    real_kwargs["arg_value"] = kwargs["args"]
        ...
        if "kwargs" in kwargs:
            if isinstance(kwargs["kwargs"], str):
                ...
                try:
                    ...
                    if cleaned_kwargs.strip().startswith(
                        "{"
                    ) and cleaned_kwargs.strip().endswith("}"):
                    ...
                    logger.warning(
                        f"Failed to parse kwargs as JSON: {kwargs['kwargs']}. Error: {str(e)}"
                    )
                ...
                logger.warning(
                    f"Received kwargs of unexpected type: {type(kwargs['kwargs'])}"
                )
    ...
    async def _execute_func_async(
        ...
        try:
            ...
            if (
                func.__name__ == "search_with_custom_fields"
                and "custom_fields" not in kwargs
            ):
            ...
            logger.debug(
                f"Calling {func.__name__} with args: {positional_args} and kwargs: {actual_kwargs}"
            )
    ...
    def register_loaded_tools(self, loaded_tools: Dict[str, Callable]) -> None:
        ...
        logger.info(
            f"Registering {len(new_tools)} new tools out of {len(loaded_tools)} total tools"
        )
        ...
        logger.info(
            f"Registered {len(new_tools)} new tools ({streaming_count} streaming, {non_streaming_count} non-streaming)"
        )
